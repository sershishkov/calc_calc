...
import { GenExampleDivBySimpleNumber } from '@/utils/generateExample';
...
export default function DivisionBySingleNumber({
  exerciseName,
}: Readonly<{
  exerciseName: string;
}>) {
  ...
  const [example, setExample] = useState<GenExampleDivBySimpleNumber>();
  ...
  const nextTask = () => {
    const newExample = new GenExampleDivBySimpleNumber(Number(max));
