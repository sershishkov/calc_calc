...
import { GenerateExampleMultDifferentRangesForOperands } from '@/utils/generateExample';
...
export default function FastMultTwoByTwo({
  exerciseName,
}: Readonly<{
  exerciseName: string;
}>) {
  ...
  const [example, setExample] =
    useState<GenerateExampleMultDifferentRangesForOperands>();
  ...
  const nextTask = () => {
    setExample(
      new GenerateExampleMultDifferentRangesForOperands(
        Number(min),
        Number(max),
        Number(min),
        Number(max)
