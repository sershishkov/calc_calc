...
import { GenExampleSquaringEnding9 } from '@/utils/generateExample';
...
export default function SquaringEnding9({
  exerciseName,
}: Readonly<{
  exerciseName: string;
}>) {
  ...
  const [example, setExample] = useState<GenExampleSquaringEnding9>();
  ...
  const nextTask = () => {
    setExample(new GenExampleSquaringEnding9(Number(max)));
