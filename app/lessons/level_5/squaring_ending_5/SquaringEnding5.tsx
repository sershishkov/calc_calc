...
import { GenExampleSquaringEnding5 } from '@/utils/generateExample';
...
export default function SquaringEnding5({
  exerciseName,
}: Readonly<{
  exerciseName: string;
}>) {
  ...
  const [example, setExample] = useState<GenExampleSquaringEnding5>();
  ...
  const nextTask = () => {
    setExample(new GenExampleSquaringEnding5(Number(max)));
